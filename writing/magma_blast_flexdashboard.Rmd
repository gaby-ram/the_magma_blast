...
for(i in 1:length(years)){
  volcano_polygon_df = data.frame(
    x = c(years[i], years[i] + .5, years[i] + .75),
    y = c(0, veis[i], 0),
...
volcano_timeline = ggplot() +
  ...
  geom_segment(aes(y = 0, yend = 0, x = 1957, xend = 2021), 
               size = 1,
               colour = "black",
               arrow = arrow()) +
  scale_x_continuous(limits = c(1957, 2021),
                     expand = c(0.005, 0.005)) +
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_manual(values = rcartocolor::carto_pal(n = 4, name = "BrwnYl")) +
  guides(fill = guide_legend(nrow = 1,
                             label.position = "bottom", 
                             keywidth = grid::unit(10, "lines"),
                             keyheight = grid::unit(1, "lines"))) +
  labs(x = NULL, y = NULL, fill = NULL,
       title = "A Timeline of Volcanic Activity Since 1960 in Japan",
  ...
  theme(plot.background = element_rect(fill = "#303030"),
        plot.title = element_text(colour = "#fff4e6", hjust = 0.5, face = "bold", size = 16),
        plot.subtitle = element_text(colour = "#B3B3B3", hjust = 0.5, face = "bold", size = 10),
        plot.caption = element_text(colour = "#B3B3B3", size = 6),
        panel.border = element_blank(),
        panel.background = element_blank(),
        panel.grid = element_blank(),
        axis.text = element_text(colour = "#B3B3B3", face = "bold", size = 14),
        axis.text.y = element_blank(),
        legend.position = "bottom",
        legend.text = element_text(colour = "#B3B3B3", size = 10),
