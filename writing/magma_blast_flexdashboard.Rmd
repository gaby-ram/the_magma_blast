...
# filter NZ data only
nzf_volc <- volcano %>% 
  filter(country %in% c("New Zealand", "Fiji")) #%>%
...
```{r include=FALSE}
# load map for NZ
nzf <- map_data("world", region = c("New Zealand", "Fiji"))
nz <- map_data("world", region = "New Zealand")
```

Change to an sf object by saying which columns are the coordinates and setting a CRS:
```{r}
nz_volc<- st_as_sf(nz_volc, coords = c("longitude", "latitude")) %>%
  st_set_crs(2193) #setting CRS
nz_volc %>% slice(1:3)
```
...
p01 <- ggplot() +
  geom_density_ridges_gradient(
    data = nz_volc,
    aes(x = elevation, y = tectonic_settings, fill = stat(x))
  ) +
  scale_fill_viridis() +
  theme_minimal() +
  theme(
...
p01 + 
  ...
  plot_annotation(
  title = "Volcanoes in New Zealand & Fiji",
  subtitle = "subtitle",
...
```{r}
# map of USA another way
usa2 <-plot_usmap(regions = "states") + 
...
```{r}
# map with tigris
usa_tigris <-regions(resolution = "500k", year = NULL, class = "sf")
class(usa_tigris)
```
